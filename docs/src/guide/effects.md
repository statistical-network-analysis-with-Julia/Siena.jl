...
Interaction effects are not implemented. `include_interaction!` throws
`ArgumentError` instead of estimating a model without the requested interaction.
Define a custom concrete effect when a scientifically validated interaction formula
is needed.
...
The built-in effects table currently supports influence of the network on the
behavior:
...
Network selection based on the evolving behavior (`:simbehavior`) is not available
through `get_effects`. `SimilarityEffect` reads an observed covariate from
`data.covariates`; it does not read the changing behavior state. Joint selection and
influence models requiring that mechanism need an explicit custom network effect
with a validated formula. Do not substitute a frozen covariate and label it the
same co-evolution model.
