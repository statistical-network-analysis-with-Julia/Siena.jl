...
```julia
alg = siena_algorithm(rng=MersenneTwister(42), conditional=true, verbose=false)
conditional_result = try
    fit_siena(data, effects; algorithm=alg)
catch err
    err isa SienaConvergenceError || rethrow()
    println(sprint(showerror, err))  # report the failed criteria
    nothing                       # do not use the unconverged fit for inference
end
if conditional_result !== nothing
    println(conditional_result.rate_estimates)
end
```

A conditional fit must pass its own independent validation; successful
unconditional estimation does not establish conditional convergence. The example
handles a convergence failure explicitly. With several dependent variables, name
the conditioning variable via
...
The checked-in fixtures were generated by RSiena 1.6.6 and include provenance
and runnable R scripts. They cover 34 deterministic effect targets (tolerance
`1e-9`) and the eight-parameter unconditional s50 model used above. Fitted
coefficients and standard errors are compared across five declared Julia seeds;
raw derivative and statistic-covariance diagonals are also checked.

Newton refinement is followed by an independent final simulation batch. All five
regression seeds must meet every `|t| < 0.1` and `tconv_max < 0.25` criterion.
An unconverged fit now throws `SienaConvergenceError` by default; its `result` field
preserves diagnostics. `allow_unconverged=true` returns it with a warning even when
`verbose=false`. Broader effect-catalogue and conditional-fit R parity remain unproven.
