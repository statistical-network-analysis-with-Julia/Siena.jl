...
Inference and interoperability updates from the July and September panel reviews.
The September changes add independent convergence validation after Newton refinement,
retain uncertainty diagnostics, and replace transcribed R targets with a generated fixture.

### Breaking

- Overall convergence now checks covariance rank in standardized moment units.
  Changing covariate units cannot erase a discrepancy in a covariance null
  direction; a nonzero discrepancy with exactly zero variance is rejected.

- Unconverged fits now throw `SienaConvergenceError` by default, including when
  progress output is disabled. The exception preserves `result` for inspection;
  `allow_unconverged=true` returns that diagnostic result with a warning instead.
- Standard errors no longer use an unconditional `0.01I` ridge. Raw derivative
  condition numbers are reported; singular or severely ill-conditioned derivatives
  produce undefined SEs and fail convergence.
- Fits snapshot their data/effects. Conditional fitting no longer fixes or rescales
  rate entries in the caller's effects table.
- Per-bin GOF p-values now use Networks' shared two-sided rank-tail convention.
- `SienaResult` additionally retains `data`, `derivative_matrix`, `phase3_cov`,
  `condition_number`, refinement counts and the actual convergence thresholds.
  Construct fitted results through `fit_siena`, not positional constructors.
...
- **Divergence is surfaced instead of silent.** Objective parameters are
  ...
  *Migration:* inspect `result.converged` and both convergence diagnostics;
  `diverged` only records whether an iterate touched a parameter clamp.
...
- Capped Newton refinement between phases 2 and 3, using score derivatives (finite
  differences for conditional models). Final diagnostics always use fresh simulations.
- A genuine RSiena 1.6.6 fixture for 34 deterministic per-effect targets, generated by
  `test/fixtures/r/s50_targets.R`; tests compare at `1e-9` instead of transcribed literals.
- `rng` on fitting, algorithm configuration, simulation and GOF; deprecated `seed`
  compatibility retained. `n_sim` is the canonical GOF simulation-count keyword.
- `siena07` is a constant alias of `fit_siena`; `gof(result)` uses saved data and
  `coeftable(result)` returns the ecosystem coefficient table with shared normal p-values.
- Sorted adjacency lists for simulation state, O(degree) transitive-triplet changes,
  and reusable `MinistepWorkspace` probability/score buffers. Independent phase-1/2
  simulation batches now honor `parallel` as well.
- Networks integration is ordinary package source; the redundant extension entry is removed.
...
- **A real RSiena golden fixture for FITTED OUTPUT, not just targets**
  ...
  The September Newton refinement now meets both convergence criteria for all
  five declared Julia seeds on an independent final batch. Raw derivative and
  phase-3 covariance diagonals are checked against the frozen R values. This is
  evidence for the specified unconditional model, not whole-catalogue equivalence.
