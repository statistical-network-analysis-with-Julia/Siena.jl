...
The final independent simulation batch did not validate a Siena fit. The diagnostic
[`SienaResult`](@ref) is available as `error.result`; it is not a converged estimate.
Set `SienaAlgorithm(allow_unconverged=true)` to return that result with a warning.
"""
...
function Base.showerror(io::IO, err::SienaConvergenceError)
    r = err.result
    print(io, "Siena fit did not converge: max |t-ratio| = ", maximum(abs, r.t_ratios),
          " (required < ", r.convergence_threshold, "), tconv_max = ", r.tconv_max,
          " (required < ", r.overall_convergence_threshold, "); derivative condition = ",
          r.condition_number, ". Inspect error.result, or set allow_unconverged=true ",
          "to return diagnostics with a warning.")
end
...
derivative matrix gives undefined standard errors — see [`approximations`](@ref).
"""
...
3. Up to `algorithm.refine_max` capped Newton updates using new score-derivative
   batches (finite differences for conditional models), followed by independent
   simulations at the final estimates for convergence t-ratios
...
`algorithm.overall_convergence_threshold` (0.25). Unconverged fits throw
[`SienaConvergenceError`](@ref) by default. With `allow_unconverged=true`, a warning
is emitted even with `verbose=false` and the diagnostic result is returned.
...
entries are marked fixed on an independent copy of the effects object — and are estimated afterwards
...
function fit_siena(data::SienaData, effects::SienaEffects;
                   algorithm::SienaAlgorithm=SienaAlgorithm(),
                   rng::AbstractRNG=algorithm.seed === nothing ? algorithm.rng : MersenneTwister(algorithm.seed))
    data.n_waves >= 2 ||
    ...
    function sim(θv)
        ...
        _run_simulations!(algorithm.n_simulations, algorithm.parallel) do i
    ...
    # maximization, so Networks.newton_fit's objective line search does not apply.
    # Stop using the refinement batch only. Final validation draws below are never
    # used to select the estimate, avoiding optional-stopping bias in diagnostics.
    n_refinements = 0
    for iteration in 1:algorithm.refine_max
        stats, scores = _refinement_batch(data, effects, pm, start_states, θ,
        ...
        d = condvar === nothing ? cov(stats, scores) :
    ...
    # Preserve the raw estimate; no ridge is added.
    ...
    if !converged
        err = SienaConvergenceError(result)
        algorithm.allow_unconverged || throw(err)
...
# Shared score/finite-difference refinement batch. These observations never become
# phase 3; keeping fresh final draws protects convergence/SE diagnostics from selection.
function _refinement_batch(data, effects, pm, start_states, θ, n, rng;
                           condvar=nothing, cond_targets=nothing, variables=nothing,
                           parallel=true, counter=nothing)
    stats = zeros(n, length(θ))
    scores = zeros(n, length(θ))
    seeds = rand(rng, 1:10^8, n)
    _run_simulations!(n, parallel) do i
        acc = condvar === nothing ? ScoreAccumulator(pm) : nothing
        stats[i, :] = _simulate_moments(data, effects, pm, start_states, θ, seeds[i];
            scores=acc, condvar=condvar, cond_targets=cond_targets,
            variables=variables, counter=counter)
        acc === nothing || (scores[i, :] = acc.scores)
    end
    return stats, scores
end
