...
- `seed::Union{Int, Nothing}`: Deprecated shim for `rng=MersenneTwister(seed)`.
- `allow_unconverged::Bool`: Return an unconverged fit with a warning when `true`.
  The default `false` throws [`SienaConvergenceError`](@ref), carrying the result.
...
mutable struct SienaAlgorithm
    ...
    function SienaAlgorithm(;
        ...
        rng = _rng_from_seed(rng, seed, :SienaAlgorithm)
        new(n_subphases, phase1_iterations, phase3_iterations,
            initial_gain, min_gain, max_iterations, convergence_threshold,
            seed, rng, allow_unconverged, refine_max, model_type, conditional, condvar, n_simulations, parallel, verbose,
...
# Keep the seed shim at API boundaries; internal code passes RNG objects directly.
function _rng_from_seed(rng::AbstractRNG, seed::Union{Int, Nothing}, caller::Symbol)
    seed === nothing && return rng
    rng === Random.default_rng() ||
        throw(ArgumentError("specify rng or seed, not both"))
    Base.depwarn("seed is deprecated; pass rng=MersenneTwister(seed) instead", caller)
