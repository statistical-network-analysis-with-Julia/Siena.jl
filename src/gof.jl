...
function siena_gof(result::SienaResult, data::SienaData, statistic::AbstractGOFStatistic;
                  n_sim::Int=100, n_sims::Union{Int, Nothing}=nothing,
                  rng::AbstractRNG=Random.default_rng(), seed::Union{Int, Nothing}=nothing)

    rng = _rng_from_seed(rng, seed, :siena_gof)
    n_sim = _gof_n_sim(n_sim, n_sims)
    ...
    simulated = zeros(Int, n_sim, n_levels)
    ...
    for s in 1:n_sim
        state_sim, _ = simulate_saom(data, result.effects, result.estimates;
                                     rng=MersenneTwister(rand(rng, 1:10^8)), variables=sim_vars)
    ...
    # the finite-simulation +1 correction), identical to Networks.GOFStatistic.
    p_values = [mc_pvalue(view(simulated, :, i), observed[i]) for i in 1:n_levels]
    ...
    sim_dists = [_mahalanobis(sim_f[s, :], center, C) for s in 1:n_sim]
    p_overall = (1 + count(>=(obs_dist), sim_dists)) / (n_sim + 1)
...
"""
    gof(result::SienaResult, data::SienaData, statistic; n_sim=100, rng=Random.default_rng())
...
function gof(result::SienaResult, data::SienaData,
    ...
    rng = _rng_from_seed(rng, seed, :gof)
    n_sim = _gof_n_sim(n_sim, n_sims)
    results = [siena_gof(result, data, stat; n_sim=n_sim, rng=rng) for stat in statistics]
    length(results) == 1 && return GOFResult(results[1])
    return GOFResult([_gof_statistic(r) for r in results]; model="SAOM")
end

function _gof_n_sim(n_sim, n_sims)
    n_sims === nothing && return n_sim
    n_sim == 100 || throw(ArgumentError("specify n_sim or n_sims, not both"))
    Base.depwarn("n_sims is deprecated; use n_sim", :siena_gof)
    return n_sims
end
...
"""
    gof(result::SienaResult; statistics=nothing, n_sim=100, rng=Random.default_rng())
    gof(result::SienaResult, statistic; kwargs...)

Assess a fitted model using its saved data. By default, compare in/outdegree
frequencies for network variables and category frequencies for behaviors.
`statistics` accepts one [`AbstractGOFStatistic`](@ref) or a vector.
"""
