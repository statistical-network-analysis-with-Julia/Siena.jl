...
function _objective_and_contrib!(col::AbstractVector{Float64}, oset::ObjectiveEffectSet,
...
"""
    MinistepWorkspace(n_alters::Int, n_parameters::Int=0)

Reusable network-choice buffers. `n_parameters` is the full free-parameter count
when collecting trajectory scores, or zero for probabilities only. Allocate one
per concurrent simulation. Reuse it with [`compute_network_choice_probs!`](@ref).
The buffers are overwritten by each call; only the first `n_candidates` entries
are active (remaining probabilities are zero).
"""
mutable struct MinistepWorkspace
    probabilities::Vector{Float64}
    alters::Vector{Int}
    contributions::Matrix{Float64}
    n_candidates::Int
end

function MinistepWorkspace(n_alters::Int, n_parameters::Int=0)
    n_alters >= 0 && n_parameters >= 0 || throw(ArgumentError("buffer sizes must be nonnegative"))
    return MinistepWorkspace(zeros(n_alters + 1), zeros(Int, n_alters + 1),
                             zeros(n_parameters, n_alters + 1), 0)
end

"""
    compute_network_choice_probs!(workspace::MinistepWorkspace, effects::ObjectiveEffectSet,
                                  theta, state, data, actor, variable; param_map=nothing)

Write network-choice probabilities and alter indices into reusable buffers and
return `workspace`. Pass the full `param_map` to also compute objective gradients
for trajectory scores. This method allocates no choice buffers; the allocating
[`compute_network_choice_probs`](@ref) remains available when owned outputs are needed.
"""
function compute_network_choice_probs!(work::MinistepWorkspace, oset::ObjectiveEffectSet,
                                      θ::Vector{Float64}, state::NetworkState,
                                      data::SienaData, actor::Int, variable::Symbol;
                                      param_map::Union{Nothing, ParameterMap}=nothing)
    net = state.networks[variable]
    dep = data.dependents[variable]::DependentNetwork
    n_alters = size(net, 2)
    length(work.probabilities) == n_alters + 1 ||
        throw(DimensionMismatch("workspace must match the number of alters"))
    param_map === nothing || size(work.contributions, 1) == n_free_parameters(param_map) ||
        throw(DimensionMismatch("workspace must match the free parameter count"))
    fill!(work.probabilities, 0.0)
    fill!(work.alters, 0)
    fill!(work.contributions, 0.0)
    work.n_candidates = 1
    onemode = dep.type == :onemode
    smask = _structural_mask(dep, state.period)
    act = state.active
    if act !== nothing && !act[actor]
        work.probabilities[1] = 1.0
        return work
    end
    n_rate = param_map === nothing ? 0 : n_free_rate_parameters(param_map)
    for alter in 1:n_alters
        onemode && alter == actor && !dep.allow_self_loops && continue
        smask !== nothing && smask[actor, alter] && continue
        act !== nothing && onemode && !act[alter] && continue
        k = work.n_candidates += 1
        work.alters[k] = alter
        work.probabilities[k] = if param_map === nothing
            compute_objective(oset, θ, state, data, actor, alter, variable)
        else
            _objective_and_contrib!(view(work.contributions, :, k), oset, θ,
                                    state, data, actor, alter, variable, n_rate)
        end
    end
    probs = work.probabilities
    max_obj = maximum(view(probs, 1:work.n_candidates))
    for k in 1:work.n_candidates
        probs[k] = exp(probs[k] - max_obj)
    end
    scale = sum(probs)
    probs ./= scale
    return work
...
# Compatibility helper returning owned outputs for callers needing them.
function _network_choice_probs_contribs(oset::ObjectiveEffectSet, θ::Vector{Float64},
                                        pm::ParameterMap, state::NetworkState,
                                        data::SienaData, actor::Int, variable::Symbol)
    work = MinistepWorkspace(size(state.networks[variable], 2), n_free_parameters(pm))
    compute_network_choice_probs!(work, oset, θ, state, data, actor, variable; param_map=pm)
    k = work.n_candidates
    return work.probabilities[1:k], work.alters[1:k], work.contributions[:, 1:k]
end
...
function _accumulate_choice_scores!(sacc::ScoreAccumulator, C::Matrix{Float64},
    ...
    for i in eachindex(sacc.scores)
        expected = 0.0
        for j in eachindex(probs)
            expected += C[i, j] * probs[j]
        end
        sacc.scores[i] += C[i, chosen_idx] - expected
    end
    return sacc
end
...
function execute_network_ministep!(state::NetworkState,
                                  ...
                                  actor::Int, variable::Symbol, rng::AbstractRNG;
                                  scores::Union{Nothing, ScoreAccumulator}=nothing,
                                  workspace::Union{Nothing, MinistepWorkspace}=nothing)
    oset = effects isa ObjectiveEffectSet ? effects : build_objective_set(effects)
    work = workspace === nothing ? MinistepWorkspace(size(state.networks[variable], 2),
                scores === nothing ? 0 : length(scores.scores)) : workspace
    compute_network_choice_probs!(work, oset, θ, state, data, actor, variable;
                                   param_map=scores === nothing ? nothing : scores.pm)
    probs, alters, C = work.probabilities, work.alters, work.contributions
...
function simulate_period!(state::NetworkState,
    ...
    # One buffer per variable and simulation; no choice arrays per ministep.
    workspaces = Dict(v => MinistepWorkspace(size(state.networks[v], 2),
                            scores === nothing ? 0 : length(scores.scores))
                      for (v, dep) in data.dependents if dep isa DependentNetwork)
    ...
    while step < max_steps
        ...
        if dep isa DependentNetwork
            change = execute_network_ministep!(state, oset, θ_obj, data, actor,
                                               ...
                                               workspace=workspaces[selected_var])
            if change > 0
...
function simulate_saom(data::SienaData, effects::SienaEffects, θ::Vector{Float64};
    ...
    r = _rng_from_seed(rng, seed, :simulate_saom)
