...
function make_fixture(n::Int; degree::Union{Nothing, Int}=nothing)
    rng = Random.Xoshiro(20260712)
    data = siena_data()
    add_nodeset!(data, NodeSet(n))
    net1 = if degree === nothing
        [Int(rand(rng) < 0.08 && i != j) for i in 1:n, j in 1:n]
    else
        [Int(mod(j - i, n) in 1:degree) for i in 1:n, j in 1:n]
    end
    net2 = copy(net1)
    for _ in 1:(5 * n)
...
@testset "Reusable network choice buffers" begin
    data, oset, theta, state = make_fixture(60)
    effects = get_effects(data)
    include_effects!(effects, :net, [:outdegree, :recip, :transTrip])
    pm = build_param_map(effects)
    work = MinistepWorkspace(60)
    scored = MinistepWorkspace(60, length(pm.free))
    compute_network_choice_probs!(work, oset, theta, state, data, 3, :net)
    compute_network_choice_probs!(scored, oset, theta, state, data, 3, :net; param_map=pm)
    @test (@allocated compute_network_choice_probs!(work, oset, theta,
                          state, data, 3, :net)) == 0
    @test (@allocated compute_network_choice_probs!(scored, oset, theta,
                          state, data, 3, :net; param_map=pm)) == 0
    old_probs, old_alters = compute_network_choice_probs(oset, theta, state, data, 3, :net)
    @test work.probabilities[1:work.n_candidates] ≈ old_probs
    @test work.alters[1:work.n_candidates] == old_alters
end

@testset "Sparse candidate-choice scaling" begin
    times = Float64[]
    for n in (240, 960)
        data, oset, theta, state = make_fixture(n; degree=4)
        work = MinistepWorkspace(n)
        compute_network_choice_probs!(work, oset, theta, state, data, 3, :net)
        elapsed = @belapsed compute_network_choice_probs!($work, $oset, $theta,
                              $state, $data, 3, :net) samples=40 seconds=0.4
        push!(times, elapsed)
    end
    ratio = times[2] / times[1]
    println("SCALING Siena sparse choice n960/n240 = ", ratio)
    @test ratio < 6.0
end
